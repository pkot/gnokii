...
static inline void NetmonOnOff (GtkWidget *w, gpointer data)
{
  ...
  if (mode)
    GSM->NetMonitor (0xf3, screen);
  else
    GSM->NetMonitor (0xf1, screen);
}
...
static inline void RefreshDisplay ()
{
  ...
  GSM->NetMonitor (displayData.curDisp, screen);
  if (screen && displayData.label)
    gtk_label_set_text (GTK_LABEL (displayData.label), g_strchomp (screen));
}


inline void SetDisplay (GtkWidget *widget, gpointer data)
{
  displayData.curDisp = GPOINTER_TO_INT (data);
  RefreshDisplay ();
}

static inline void ChangedSpinner (GtkWidget *widget, GtkSpinButton *spin)
{
...
void GUI_RefreshNetmon ()
{
  ...
  ParseScreen (screen, 0);
      
  GSM->NetMonitor (4, screen);
  ParseScreen (screen, 3);
  
  GSM->NetMonitor (5, screen);
  ParseScreen (screen, 6);
...
static GtkItemFactoryEntry menu_items[] = {
  ...
  {"/File/_Close",	"<control>W",	CloseNetmon, 0, NULL},
