                                   ...
                                   SelectContactData *data)
{
  ...
  {
    GSM_SpeedDial *entry = (GSM_SpeedDial *) g_malloc (sizeof (GSM_SpeedDial));
    ...
    entry->Number = *key - '0';
    entry->MemoryType = pbEntry->entry.MemoryType + 2;
    entry->Location = pbEntry->entry.Location;
    
    g_free ((GSM_SpeedDial *) gtk_clist_get_row_data (GTK_CLIST (clist),
            selectedKey));
    gtk_clist_set_row_data_full (GTK_CLIST (clist), selectedKey,
                                 (gpointer) entry, DestroyCListData);
...
static void ReadSpeedDial (void)
{
  ...
  for (i = 1; i < 10; i++)
  {
    if ((entry = (GSM_SpeedDial *) g_malloc (sizeof (GSM_SpeedDial))) == NULL)
    {
    ...
    else
    {
      ...
      if ((pbEntry = GUI_GetEntry (entry->MemoryType - 2, location)) == NULL)
      {
        g_free (entry);
        continue;
      }
...
static void SaveSpeedDial (void)
{
  ...
  if (speedDialInitialized)
    for(i = 1; i < 10; i++)
      if ((entry = (GSM_SpeedDial *) gtk_clist_get_row_data (GTK_CLIST (clist), i - 1)))
      {
        if (entry->Location == 0)
          continue;
...
static bool ParseLine (GSM_SpeedDial *entry, gchar *buf)
{
  gchar **strings = g_strsplit (buf, ";", 3);
  
  entry->Number = *strings[0] - '0';
  ...
  entry->MemoryType = *strings[1] - '0';
  if (entry->MemoryType < 2 || entry->MemoryType > 3)
  {
    g_strfreev (strings);
    return FALSE;
  }
  
  entry->Location = atoi (strings[2]);
  if (entry->Location == LONG_MIN || entry->Location == LONG_MAX ||
      entry->Location < 0)
  {
    g_strfreev (strings);
    return FALSE;
...
static void ExportSpeedDialMain (gchar *name)
{
  ...
  for(i = 1; i < 10; i++)
  {
    if ((entry = (GSM_SpeedDial *) gtk_clist_get_row_data (GTK_CLIST (clist), i - 1)))
    {
      sprintf (buf, "%d;%d;%d;", entry->Number, entry->MemoryType, entry->Location);
      fprintf (f, "%s\n", buf);
    }
  }
...
static GtkItemFactoryEntry menu_items[] = {
  ...
  {"/File/_Read from phone", "<control>R", ReadSpeedDial, 0, NULL},
  {"/File/_Save to phone", "<control>S", SaveSpeedDial, 0, NULL},
  {"/File/sep1",	NULL,		NULL, 0, "<Separator>"},
  {"/File/_Import from file", "<control>I", ImportSpeedDial, 0, NULL},
  {"/File/_Export to file", "<control>E", ExportSpeedDial, 0, NULL},
  {"/File/sep2",	NULL,		NULL, 0, "<Separator>"},
  {"/File/_Close",	"<control>W",	CloseSpeedDial, 0, NULL},
  {"/_Edit",		NULL,		NULL, 0, "<Branch>"},
  {"/Edit/_Edit",	NULL,		EditKey, 0, NULL},
