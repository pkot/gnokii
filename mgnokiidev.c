/*
	...
	mgnokiidev.c - Gets passed a slave pty number by gnokiid and uses
	this information to create a symlink from the pty to /dev/gnokii.

    Using a number rather than a partial device name is more secure
	but may be limiting on some systems.  If it is on yours, please
	tell me and/or submit a patch...
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		...
		/* Check is a reasonable length (less than 8 characters) */
	if (strlen(argv[1]) >= 8) {
	...
	if (err < 0) {
		...
		/* Possible bug - should check that /dev/gnokii doesn't already exist
		   in case multiple users are trying to run gnokii */

		/* Remove symlink in case it already exists.  Don't care
		    if it fails.  */
	unlink ("/dev/gnokii");
